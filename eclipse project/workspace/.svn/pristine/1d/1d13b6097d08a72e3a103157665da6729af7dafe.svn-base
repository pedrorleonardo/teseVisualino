...
public abstract class FarruscoCompileStandAlone {
	...
	protected EmfModel createEmfModelByURI(String name, String model, 
		...
		properties.put(EmfModel.PROPERTY_FILE_BASED_METAMODEL_URI, "false");
		properties.put(EmfModel.PROPERTY_READONLOAD, readOnLoad + "");
		properties.put(EmfModel.PROPERTY_STOREONDISPOSAL, 
				storeOnDisposal + "");
		emfModel.load(properties, null);
	...
	protected File getFile(String fileName) throws URISyntaxException {
		...
		URL uri = null;
		try {
			binUri = FarruscoCompileStandAlone.class.
						getResource(fileName).toURI().toURL();
			
			URL realURI = FileLocator.toFileURL(binUri);
			if (realURI.toString().indexOf("bin") > -1) {
				uri = new URL(realURI.toString().replaceAll("bin", "src"));
			}
			else {
				uri = realURI;
			}
